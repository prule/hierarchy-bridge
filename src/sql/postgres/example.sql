...
-- all from D and above
select parent_id
from hierarchies_bridge
where child_id = '4';

-- all from D and above
select h.hierarchy_id, h.hierarchy_name
from hierarchies h
         join hierarchies_bridge hb on h.hierarchy_id = hb.parent_id
where hb.child_id = '4'
;

-- all from B and below
select h.hierarchy_id, h.hierarchy_name
from hierarchies h
         join hierarchies_bridge hb on h.hierarchy_id = hb.child_id
where hb.parent_id = '2'
;
